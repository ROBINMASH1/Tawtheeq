    ...
    <div className="bg-white dark:bg-gray-900 min-h-screen pt-20">
      ...
      <section className="px-8 py-24 bg-gray-50 dark:bg-gray-800/50">
        <div className="max-w-7xl mx-auto">
          <ScrollReveal direction="bottom" y={35}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: <img src="/padlock.png" alt="Immutability" className="w-10 h-10 object-contain" />, title: 'Immutability', desc: 'Once a credential is cryptographically signed and hashed on the blockchain, it cannot be altered, tampered with, or deleted. It serves as a permanent source of truth.' },
              ...
              <ScrollReveal key={i} direction={i === 0 ? 'left' : i === 2 ? 'right' : 'bottom'} delay={i * 0.15}>
                <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 hover:-translate-y-1 transition-transform">
                  <div className="text-4xl mb-6 flex items-center">{feature.icon}</div>
      ...
      <section className="px-8 py-24">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 items-center">
          <ScrollReveal direction="left" className="flex-1 flex flex-col gap-8">
            <div>
              ...
              {[
                {
                  ...
                  icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" /></svg>,
                  title: 'Hyperledger Fabric',
                  desc: 'A permissioned enterprise-grade blockchain network providing maximum transaction speed, private data collections, and robust security controls suitable for government and academic entities.',
                },
                {
                  color: 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400',
                  icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>,
                  title: 'Chaincode (Smart Contracts)',
                  desc: 'Automated, self-executing business logic written in Node.js that handles the issuance, validation, and revocation rules on-chain, ensuring absolute trust and consistency across all university nodes.',
                },
                {
                  color: 'bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400',
                  icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" /></svg>,
                  title: 'IPFS Integration',
                  desc: 'To keep the blockchain fast and cheap, heavy credential files (like PDFs) are encrypted and stored on the InterPlanetary File System (IPFS). Only the lightweight cryptographic hash is stored on-chain.',
                },
                {
                  color: 'bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400',
                  icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>,
                  title: 'Cryptographic Hashing',
                  desc: 'Every certificate is run through a SHA-256 algorithm to generate a unique hash. If even a single pixel or letter of the certificate is changed, the hash completely changes, instantly flagging the document as forged.',
                },
              ].map((item, i) => (
                <ScrollReveal key={i} direction="left" delay={i * 0.15} y={25}>
                  <div className="flex items-start gap-4">
                    <div className={`w-12 h-12 rounded-xl ${item.color} flex items-center justify-center shrink-0 mt-1`}>
                      {item.icon}
                    </div>
                    <div>
          ...
          <ScrollReveal direction="right" delay={0.2} className="flex-1 w-full max-w-md lg:max-w-none">
