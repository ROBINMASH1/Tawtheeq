...
export default function UniversityManagement() {
  ...
  const filteredUniversities = universities.filter((uni) => {
    const matchSearch =
      ...
      uni.Initialism?.toLowerCase().includes(search.toLowerCase());
    const matchFilter =
      filterStatus === "all" ||
      (filterStatus === "active" && uni.isActive) ||
      (filterStatus === "inactive" && !uni.isActive);
  ...
  <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
    <main className="pt-20 px-8 max-w-7xl mx-auto pb-16">
      ...
      >
        ...
        {/* Back Button (kept left, not affecting centering) */}
        {(view === "create" || view === "edit") && (
          <button
            onClick={() => { setView("list"); resetForm(); }}
            className="absolute left-0 top-0 w-10 h-10 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white flex items-center justify-center shadow-sm transition-colors"
          >
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className={`text-sm text-gray-500 dark:text-gray-400 mt-1 ${
            ...
            {view === "list"
              ? `${universities.length} registered ${universities.length === 1 ? "university" : "universities"}`
              : view === "create"
        ...
        {/* Right button only in list view (unchanged) */}
        {view === "list" && (
          <button
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl px-5 py-4 flex flex-col sm:flex-row gap-3 shadow-sm">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search by name, email, license, org ID..."
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 gap-3">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 font-medium">
                    {search || filterStatus !== "all" ? "No universities match your search." : "No universities yet."}
                  </p>
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    {search || filterStatus !== "all" ? "Try adjusting your filters." : "Create the first university to get started."}
                  </p>
                </div>
              ) : (
                <div className="divide-y divide-gray-100 dark:divide-gray-800">
                  {filteredUniversities.map((uni, i) => (
                    <div key={i} className="px-6 py-5 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        <div className="flex items-start gap-4 flex-1 min-w-0">
                          ...
                          <div className="flex flex-col gap-3 flex-1 min-w-0">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-3">
                              ...
                              {uni.adminIdentifier && (
                                <InfoRow label="Admin Identifier" value={uni.adminIdentifier} />
                              )}
                              {uni.totalCertificates !== undefined && (
                                <InfoRow label="Total Certificates" value={uni.totalCertificates} />
                              )}
                              {uni.totalStudents !== undefined && (
                                <InfoRow label="Total Students" value={uni.totalStudents} />
                              )}
                            </div>
                          </div>
                        </div>
        ...
        {/* ── CREATE / EDIT FORM ── */}
        {(view === "create" || view === "edit") && (
          <div className="mt-6 max-w-4xl mx-auto " style={{ animation: "fadeSlideIn 0.4s ease forwards" }}>
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {[
                  { label: "University Name", key: "name", placeholder: "e.g. University of Jordan", type: "text", full: true },
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 bg-blue-600 hover:bg-blue-700 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl flex items-center justify-center gap-2 transition-all duration-200"
                >
                  {formLoading ? (
                    <>
